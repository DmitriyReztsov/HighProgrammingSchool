from lessons.OOAP.TwoWayList import TwoWayList


def test_create_boundedstack():
    tw_list = TwoWayList().TwoWayList()

    assert tw_list.get_head_status() == 0
    assert tw_list.get_left_status() == 0
...
def test_put():
    ...
    tw_list.put_left("left")
    assert tw_list.get_put_left_status() == 1
    assert tw_list._cursor == 2
    assert tw_list._head == 1
    assert tw_list._tail == 5
    assert tw_list._list == ["left", "12", 11, "right", "tail"]

    tw_list.tail()
    tw_list.put_left("left tail")
    assert tw_list.get_put_left_status() == 1
    assert tw_list._cursor == 6
    assert tw_list._head == 1
    assert tw_list._tail == 6
    assert tw_list._list == ["left", "12", 11, "right", "left tail", "tail"]
