def _collect_python_list(l_list):
    ...
    while current_node:
        ...
        # if current_node is l_list.tail:
        #     break
        current_node = current_node.next
    return values_list


def test_find(setup_instances_to_find_3):
    instances_list = setup_instances_to_find_3
...
def test_find_all(setup_instances_to_find_all_3):
    instances_list = setup_instances_to_find_all_3
    for data, find_value, check_result in instances_list:
...
def test_delete(setup_instances_to_delete_3):
    instances_list = setup_instances_to_delete_3
...
def test_insert(setup_instances_to_insert_3):
    instances_list = setup_instances_to_insert_3
...
def test_add_in_tail(setup_instances_to_add_in_tail):
    instances_list = setup_instances_to_add_in_tail
    for data, new_node, head, tail in instances_list:
        check_values_list = _collect_python_list(data)
        check_prev = data.tail
        check_next = None

        data.add_in_tail(new_node)
        test_values_list = _collect_python_list(data)
...
def test_clean(setup_instances_to_clean_3):
    instances_list = setup_instances_to_clean_3
