...
class SimpleGraph:
    ...
    @property
    def empty_vertex_index(self) -> int:
    ...
    def WeakVertices(self) -> List[Vertex]:
        ...
        non_triangle_vertex_list = []
        for v_index, vertex in enumerate(self.vertex):
            if vertex is None:
                continue
            if not self._check_triangle(v_index):
                non_triangle_vertex_list.append(vertex)
        return non_triangle_vertex_list

    def _check_triangle(self, v_index: int) -> bool:
        v_children_indexes = self._collect_children(v_index)
        for vertex_index in v_children_indexes:
            ch_children_indexes = self._collect_children(vertex_index)
            if set(v_children_indexes).intersection(set(ch_children_indexes)):
                return True
        return False

    def _collect_children(self, v_index: int) -> List:
        v_children_indexes = []
        for vertex_index, is_edge in enumerate(self.m_adjacency[v_index]):
            if not is_edge or vertex_index == v_index:
                continue
            v_children_indexes.append(vertex_index)
        return v_children_indexes
