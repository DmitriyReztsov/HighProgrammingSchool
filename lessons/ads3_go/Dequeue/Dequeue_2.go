...
// 4.4, 4.5 заведен словарь, как и в эталонном решении. При парсинге строки смотрим, есть ли скобка среди ключей (закрывающих скобок). Если нет - скобка открывающая (или мусор) - кладем в стек. Если есть среди ключей и значение ключа 
// соответствует верхнему элементу стека - удаляем из стека и смотрим дальше - до первого расхождения или до момента, когда 
// стек не окажется пустым (т.е. пытаемся применить закрывающую скобку без каких-либо открывающих). После проверяем, что 
// стек пустой (т.е. не осталось октрывающих скобок без закрывающих).

// 4.6 как я писал - приведенный эталонный алгоритм эффективен по памяти (меньше требует массива), но добавляем лишнюю 
// проверку (т.е. выше цикломатическая сложность).

// 4.7 а тут я подошел по инерции и зря ввел новый массив. Действительно, простая переменная с О(1) по памяти эффективнее.

// 4.8 в моей функции извлечение происходит синхронно, из стека, куда я перекладываю элементы из начального стека. И 
// операции производятся по switch case, что по сути аналогично словарю. В целом пока не очень понятно, как в рамках 
// последовательных действий и синхронного подхода может быть нарушен порядок аргументов. В асинхронном подходе возможна 
// такая ситуация - надо делать блокировку.
